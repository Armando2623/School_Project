(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const s of i.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function r(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerPolicy&&(i.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?i.credentials="include":o.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(o){if(o.ep)return;o.ep=!0;const i=r(o);fetch(o.href,i)}})();const we="sg_session",b={save(e){sessionStorage.setItem(we,JSON.stringify(e))},get(){try{return JSON.parse(sessionStorage.getItem(we))}catch{return null}},clear(){sessionStorage.removeItem(we)},token(){var e;return((e=this.get())==null?void 0:e.token)??null},usuario(){var e;return((e=this.get())==null?void 0:e.usuario)??null},nombre(){var e;return((e=this.get())==null?void 0:e.nombre)??null},rol(){var e;return((e=this.get())==null?void 0:e.rol)??null},id(){var e;return((e=this.get())==null?void 0:e.id)??null},isLogged(){return!!this.token()},hasRole(...e){return e.includes(this.rol())}},rt=[{section:"PRINCIPAL",items:[{icon:"fas fa-chart-line",label:"Dashboard",hash:"#/dashboard",roles:null},{icon:"fas fa-clipboard-list",label:"Visitas",hash:"#/visitas",roles:["ADMINISTRADOR","PORTERO","SECRETARIA","DIRECTOR","PROFESOR"]}]},{section:"GESTIÓN",items:[{icon:"fas fa-users",label:"Visitantes",hash:"#/visitantes",roles:["ADMINISTRADOR","SECRETARIA"]},{icon:"fas fa-user-graduate",label:"Alumnos",hash:"#/alumnos",roles:["ADMINISTRADOR","SECRETARIA","PROFESOR"]},{icon:"fas fa-check-square",label:"Asistencia",hash:"#/asistencia",roles:["ADMINISTRADOR","DIRECTOR","SECRETARIA","PORTERO","PROFESOR"]},{icon:"fas fa-calendar-alt",label:"Agenda",hash:"#/agenda",roles:["ADMINISTRADOR","DIRECTOR","SECRETARIA","PORTERO","PROFESOR"]},{icon:"fas fa-user-cog",label:"Usuarios",hash:"#/usuarios",roles:["ADMINISTRADOR"]},{icon:"fas fa-history",label:"Auditoría",hash:"#/auditoria",roles:["ADMINISTRADOR"]}]}],nt={ADMINISTRADOR:"Administrador",PORTERO:"Portero",SECRETARIA:"Secretaria",DIRECTOR:"Director",PROFESOR:"Profesor"};function lt(e){const t=b.rol(),r=window.location.hash||"#/dashboard",a=rt.map(o=>{const i=o.items.filter(s=>!s.roles||s.roles.includes(t));return i.length?`
    ...
    </aside>`,e.querySelector("#logout-btn").addEventListener("click",()=>{b.clear(),window.location.hash="#/login"})}const dt="https://school-project-1mso.onrender.com",ct="https://school-project-assitencia-service.onrender.com",ut="https://school-project-agendaservice.onrender.com",le=`${dt}/api`,xe=`${ct}/api`,be=`${ut}/api`;async function k(e,t,r={}){const a=b.token(),o={"Content-Type":"application/json",...r.headers};a&&(o.Authorization=`Bearer ${a}`);const i=await fetch(`${e}${t}`,{...r,headers:o});if(i.status===401)throw b.clear(),window.location.hash="#/login",new Error("Sesión expirada");if(i.status===204)return null;if(!i.ok){const s=await i.json().catch(()=>({error:`Error ${i.status}`}));throw new Error(s.error||s.message||`Error ${i.status}`)}return i.json()}const h={get:e=>k(le,e),post:(e,t)=>k(le,e,{method:"POST",body:JSON.stringify(t)}),put:(e,t)=>k(le,e,{method:"PUT",body:JSON.stringify(t)}),delete:e=>k(le,e,{method:"DELETE"}),fetchBlob:async e=>{const t=b.token(),r={};t&&(r.Authorization=`Bearer ${t}`);const a=await fetch(`${le}${e}`,{headers:r});if(!a.ok)throw new Error(`Error ${a.status}`);return a.blob()}},O={get:e=>k(xe,e),post:(e,t)=>k(xe,e,{method:"POST",body:JSON.stringify(t)})},S={get:e=>k(be,e),post:(e,t)=>k(be,e,{method:"POST",body:JSON.stringify(t)}),put:(e,t)=>k(be,e,{method:"PUT",body:JSON.stringify(t)}),delete:e=>k(be,e,{method:"DELETE"})},pt=Object.freeze(Object.defineProperty({__proto__:null,agendaApi:S,api:h,astApi:O},Symbol.toStringTag,{value:"Module"})),mt={login:(e,t)=>h.post("/auth/login",{usuario:e,contraseña:t})};function p(e,t="info",r=3500){const a=document.getElementById("toast-container"),o=document.createElement("div"),i={success:"✅",error:"❌",warning:"⚠️",info:"ℹ️"};o.className=`toast toast-${t}`,o.innerHTML=`<span>${i[t]??"ℹ️"}</span><span>${e}</span>`,a.appendChild(o),setTimeout(()=>{o.classList.add("leaving"),setTimeout(()=>o.remove(),310)},r)}function bt(e){e.innerHTML=`
...
<div class="svc-row">
    ...
    </div>`;const t=b.token(),r=t?{Authorization:`Bearer ${t}`}:{},a=async(o,i,s={})=>{const n=e.querySelector(`#${i}`);try{const l=await fetch(o,{signal:AbortSignal.timeout(3e3),headers:s});n.classList.add(l.status<500?"online":"offline")}catch{n.classList.add("offline")}};a("/api/alumnos","dot-mvc",r),a("/asistencia-api/asistencia","dot-ast",r);try{const[o,i]=await Promise.all([Z.listar(),pe.listar()]),s=o.filter(c=>{var u;return(u=c.horaIngreso)==null?void 0:u.startsWith(De())}),n=o.filter(c=>c.estadoRegistro==="EN_CURSO");e.querySelector("#s-activos").textContent=n.length,e.querySelector("#s-hoy").textContent=s.length,e.querySelector("#s-alumnos").textContent=i.length,e.querySelector("#footer-count").textContent=`${s.length} hoy`;const l=e.querySelector("#recent-tbody"),d=[...o].reverse().slice(0,5);d.length?l.innerHTML=d.map(c=>`
        <tr>
    ...
    </div>`,l.classList.add("active"),document.body.appendChild(l);const d=()=>{n&&n(),l.remove()};return l.querySelector("#modal-close-btn").addEventListener("click",d),(c=l.querySelector("#modal-cancel-btn"))==null||c.addEventListener("click",d),l.querySelector("#modal-confirm-btn").addEventListener("click",()=>{r?r(l,d):d()}),l.addEventListener("click",u=>{u.target===l&&d()}),s&&setTimeout(()=>s(l),0),l}const ht=["REGISTRADO","EN_CURSO","COMPLETADO"],yt={REGISTRADO:"badge-blue",EN_CURSO:"badge-yellow",COMPLETADO:"badge-green"},Et=e=>`<span class="badge ${yt[e]??"badge-gray"}">${e??"—"}</span>`,St=e=>e?new Date(e).toLocaleString("es-PE"):"—",fe=()=>b.hasRole("ADMINISTRADOR","PORTERO","SECRETARIA");let ue=[];async function $t(e){var t,r;e.innerHTML=`
    <div class="page-topbar">
      <div>
    ...
    </div>`,await Ne(e),(t=e.querySelector("#search"))==null||t.addEventListener("input",a=>{const o=a.target.value.toLowerCase();Pe(e,ue.filter(i=>{var s,n,l;return((s=i.nombreVisitante)==null?void 0:s.toLowerCase().includes(o))||((n=i.dniVisitante)==null?void 0:n.toLowerCase().includes(o))||((l=i.motivo)==null?void 0:l.toLowerCase().includes(o))}))}),(r=e.querySelector("#btn-new"))==null||r.addEventListener("click",()=>He(e))}async function Ne(e){try{ue=await Z.listar(),e.querySelector("#footer-count").textContent=`${ue.length} registros`,Pe(e,ue)}catch(t){p(t.message,"error")}}function Pe(e,t){const r=e.querySelector("#tbody");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="7">No hay registros</td></tr>';return}r.innerHTML=t.map(a=>{var o;return`
    <tr>
      <td><strong>${a.nombreVisitante}</strong></td>
      <td class="td-muted">${a.dniVisitante}</td>
      <td>${a.motivo}</td>
      <td>${((o=a.usuario)==null?void 0:o.nombre)??"—"}</td>
      <td class="td-small">${St(a.horaIngreso)}</td>
      <td>${Et(a.estadoRegistro)}</td>
      ${fe()?`<td><button class="btn btn-outline btn-sm edit-btn" data-id="${a.id}">✏️ Editar</button></td>`:""}
    </tr>`}).join(""),fe()&&r.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>{He(e,ue.find(o=>o.id==a.dataset.id))}))}async function He(e,t=null){const r=!!t;let a=[];try{a=await Z.buscarUsuarios("")}catch{}const o=new Date().toISOString().slice(0,16);N({title:r?"✏️ Editar Visita":"+ Nueva Visita",bodyHTML:`
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select class="form-control" id="m-estado">
            ${ht.map(i=>`<option value="${i}" ${((t==null?void 0:t.estadoRegistro)??"REGISTRADO")===i?"selected":""}>${i}</option>`).join("")}
          </select>
        </div>
        <div class="form-group">
      ...
      </div>`,confirmText:r?"Actualizar":"Registrar",onConfirm:async(i,s)=>{const n=i.querySelector("#m-dni").value.trim(),l=i.querySelector("#m-nombre").value.trim(),d=i.querySelector("#m-motivo").value.trim(),c=i.querySelector("#m-usuario").value,u=i.querySelector("#m-estado").value,m=i.querySelector("#m-hora").value;if(!n||!l||!d||!c){p("Completa los campos obligatorios","warning");return}const g={dniVisitante:n,nombreVisitante:l,motivo:d,usuario_id:Number(c),estadoRegistro:u,horaIngreso:m?new Date(m).toISOString():null};try{r?await Z.actualizar(t.id,g):await Z.registrar(g),p(r?"Visita actualizada":"Visita registrada","success"),s(),await Ne(e)}catch(f){p(f.message,"error")}}}),setTimeout(()=>{var i;(i=document.querySelector("#btn-dni"))==null||i.addEventListener("click",async()=>{var n;const s=(n=document.querySelector("#m-dni"))==null?void 0:n.value.trim();if(s)try{const l=await Z.buscarPorDni(s);l?(document.querySelector("#m-nombre").value=l.nombreVisitante??"",p("Datos autocompletados","info")):p("DNI no encontrado","warning")}catch{p("DNI no encontrado","warning")}})},100)}const ve={listar:()=>h.get("/visitantes"),registrar:e=>h.post("/visitantes",e),actualizar:(e,t)=>h.put(`/visitantes/${e}`,t),buscarPorDni:e=>h.get(`/visitantes/buscar?dni=${encodeURIComponent(e)}`)},ge=()=>b.hasRole("ADMINISTRADOR","SECRETARIA");let ee=[];async function Rt(e){var t;e.innerHTML=`
    <div class="page-topbar">
      <div>
        <h1>Visitantes</h1>
    ...
    </div>`,await ke(e),e.querySelector("#search").addEventListener("input",r=>{const a=r.target.value.toLowerCase();Ve(e,ee.filter(o=>{var i,s;return((i=o.dniVisitante)==null?void 0:i.toLowerCase().includes(a))||((s=o.nombreVisitante)==null?void 0:s.toLowerCase().includes(a))}))}),(t=e.querySelector("#btn-new"))==null||t.addEventListener("click",()=>Be(e))}async function ke(e){try{ee=await ve.listar(),e.querySelector("#total").textContent=`${ee.length}`,e.querySelector("#footer-lbl").textContent=`${ee.length} visitantes registrados`,Ve(e,ee)}catch(t){p(t.message,"error")}}function Ve(e,t){const r=e.querySelector("#tbody");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="6">Sin visitantes registrados</td></tr>';return}r.innerHTML=t.map(a=>{var o;return`
    <tr>
    ...
    </tr>`}).join(""),ge()&&r.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>Be(e,ee.find(o=>o.id==a.dataset.id))))}function Be(e,t=null){N({title:t?"✏️ Editar Visitante":"+ Nuevo Visitante",bodyHTML:`
      <div class="form-grid">
        <div class="form-group">
          <label>DNI *</label>
      ...
      </div>`,confirmText:t?"Actualizar":"Registrar",onConfirm:async(r,a)=>{const o={dniVisitante:r.querySelector("#m-dni").value.trim(),nombreVisitante:r.querySelector("#m-nombre").value.trim(),telefono:r.querySelector("#m-tel").value.trim()||null,email:r.querySelector("#m-email").value.trim()||null};if(!o.dniVisitante||!o.nombreVisitante){p("DNI y nombre son obligatorios","warning");return}try{t?await ve.actualizar(t.id,o):await ve.registrar(o),p(t?"Visitante actualizado":"Visitante registrado","success"),a(),await ke(e)}catch(i){p(i.message,"error")}}})}const he=()=>b.hasRole("ADMINISTRADOR","SECRETARIA");let te=[],Fe=[];async function wt(e){var t;e.innerHTML=`
    <div class="page-topbar">
      <div>
        <h1>Alumnos</h1>
    ...
    </div>`,await je(e),e.querySelector("#search").addEventListener("input",r=>{const a=r.target.value.toLowerCase();Ue(e,te.filter(o=>{var i,s,n;return((i=o.nombre)==null?void 0:i.toLowerCase().includes(a))||((s=o.grado)==null?void 0:s.toLowerCase().includes(a))||((n=o.seccion)==null?void 0:n.toLowerCase().includes(a))}))}),(t=e.querySelector("#btn-new"))==null||t.addEventListener("click",()=>ze(e))}async function je(e){try{[te,Fe]=await Promise.all([pe.listar(),ve.listar()]),e.querySelector("#total").textContent=`${te.length}`,e.querySelector("#footer-lbl").textContent=`${te.length} alumnos registrados`,Ue(e,te)}catch(t){p(t.message,"error")}}function Ue(e,t){const r=e.querySelector("#tbody");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="6">Sin alumnos registrados</td></tr>';return}r.innerHTML=t.map(a=>{var s,n;const i=!!((s=a.apoderado)!=null&&s.email)?`<span class="badge badge-green" title="El apoderado ${a.apoderado.nombreVisitante} recibirá notificaciones en ${a.apoderado.email}" style="cursor:default">📧 Email</span>`:'<span class="td-muted" title="El apoderado no tiene email registrado">—</span>';return`
    <tr>
    ...
    </tr>`}).join(""),he()&&r.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>ze(e,te.find(o=>o.id==a.dataset.id)))),r.querySelectorAll(".qr-btn").forEach(a=>a.addEventListener("click",()=>At(a.dataset.id,a.dataset.nombre)))}async function At(e,t){let r=null;N({title:`📱 Código QR — ${t}`,bodyHTML:`
      ...
      </div>`,confirmText:"Cerrar",hideCancelBtn:!0,onOpen:async a=>{try{const o=await pe.obtenerQrBlob(e);r=URL.createObjectURL(o);const i=a.querySelector("#qr-img");i.src=r,i.style.display="block",a.querySelector("#qr-loading").style.display="none",a.querySelector("#btn-download-qr").addEventListener("click",()=>{const s=document.createElement("a");s.href=r,s.download=`qr-alumno-${t.replace(/\s+/g,"-")}.png`,s.click()})}catch{a.querySelector("#qr-loading").textContent="Error al cargar el QR",p("No se pudo cargar el código QR","error")}},onClose:()=>{r&&URL.revokeObjectURL(r)}})}function ze(e,t=null){const r=Fe.map(a=>{var o;return`<option value="${a.id}" ${((o=t==null?void 0:t.apoderado)==null?void 0:o.id)===a.id?"selected":""}>${a.nombreVisitante} (${a.dniVisitante})</option>`}).join("");N({title:t?"✏️ Editar Alumno":"+ Nuevo Alumno",bodyHTML:`
      <div class="form-grid">
        <div class="form-group" style="grid-column:1/-1">
          <label>Nombre completo *</label>
      ...
      </div>`,confirmText:t?"Actualizar":"Registrar",onConfirm:async(a,o)=>{const i={nombre:a.querySelector("#m-nombre").value.trim(),grado:a.querySelector("#m-grado").value.trim(),seccion:a.querySelector("#m-seccion").value.trim(),visitanteId:a.querySelector("#m-apoderado").value||null};if(!i.nombre||!i.grado||!i.seccion){p("Nombre, grado y sección son obligatorios","warning");return}i.visitanteId&&(i.visitanteId=Number(i.visitanteId));try{t?await pe.actualizar(t.id,i):await pe.registrar(i),p(t?"Alumno actualizado":"Alumno registrado — QR generado automáticamente","success"),o(),await je(e)}catch(s){p(s.message,"error")}}})}const oe={listar:()=>h.get("/usuarios"),registrar:e=>h.post("/usuarios",e),actualizar:(e,t)=>h.put(`/usuarios/${e}`,t),eliminar:e=>h.delete(`/usuarios/${e}`)},It=["ADMINISTRADOR","PORTERO","SECRETARIA","DIRECTOR","PROFESOR"],Lt={ADMINISTRADOR:"badge-red",PORTERO:"badge-blue",SECRETARIA:"badge-green",DIRECTOR:"badge-yellow",PROFESOR:"badge-purple"},Tt=e=>`<span class="badge ${Lt[e]??"badge-gray"}">${e}</span>`;let Q=[];async function Ot(e){e.innerHTML=`
    <div class="page-topbar">
      <div>
    ...
    </div>`,await Te(e),e.querySelector("#search").addEventListener("input",t=>{const r=t.target.value.toLowerCase();_e(e,Q.filter(a=>{var o,i;return((o=a.nombre)==null?void 0:o.toLowerCase().includes(r))||((i=a.usuario)==null?void 0:i.toLowerCase().includes(r))}))}),e.querySelector("#btn-new").addEventListener("click",()=>Ge(e))}async function Te(e){try{Q=await oe.listar(),e.querySelector("#total").textContent=`${Q.length}`,e.querySelector("#footer-lbl").textContent=`${Q.length} usuarios`,_e(e,Q)}catch(t){p(t.message,"error")}}function _e(e,t){const r=e.querySelector("#tbody");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="4">Sin usuarios registrados</td></tr>';return}r.innerHTML=t.map(a=>`
    <tr>
      <td><strong>${a.nombre}</strong></td>
    ...
    </tr>`).join(""),r.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>Ge(e,Q.find(o=>o.id==a.dataset.id)))),r.querySelectorAll(".del-btn").forEach(a=>a.addEventListener("click",()=>xt(e,Q.find(o=>o.id==a.dataset.id))))}function Ge(e,t=null){const r=It.map(a=>`<option value="${a}" ${(t==null?void 0:t.rol)===a?"selected":""}>${a}</option>`).join("");N({title:t?"✏️ Editar Usuario":"+ Nuevo Usuario",bodyHTML:`
      <div class="form-grid">
        <div class="form-group">
          <label>Nombre completo *</label>
      ...
      </div>`,confirmText:t?"Actualizar":"Crear Usuario",onConfirm:async(a,o)=>{const i={nombre:a.querySelector("#m-nombre").value.trim(),usuario:a.querySelector("#m-usuario").value.trim(),contraseña:a.querySelector("#m-pass").value,rol:a.querySelector("#m-rol").value};if(!i.nombre||!i.usuario||!i.rol){p("Nombre, usuario y rol son obligatorios","warning");return}if(!t&&!i.contraseña){p("La contraseña es obligatoria","warning");return}try{t?await oe.actualizar(t.id,i):await oe.registrar(i),p(t?"Usuario actualizado":"Usuario creado","success"),o(),await Te(e)}catch(s){p(s.message,"error")}}})}function xt(e,t){N({title:"🗑 Eliminar Usuario",bodyHTML:`<p style="color:var(--text2);line-height:1.7">¿Estás seguro de eliminar a <strong style="color:var(--text)">${t.nombre}</strong> (@${t.usuario})?<br/>Esta acción no se puede deshacer.</p>`,confirmText:"Eliminar",danger:!0,onConfirm:async(r,a)=>{try{await oe.eliminar(t.id),p("Usuario eliminado","success"),a(),await Te(e)}catch(o){p(o.message,"error")}}})}const Dt="modulepreload",qt=function(e){return"/"+e},qe={},Ce=function(t,r,a){let o=Promise.resolve();if(r&&r.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),n=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));o=Promise.allSettled(r.map(l=>{if(l=qt(l),l in qe)return;qe[l]=!0;const d=l.endsWith(".css"),c=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${c}`))return;const u=document.createElement("link");if(u.rel=d?"stylesheet":Dt,d||(u.as="script"),u.crossOrigin="",u.href=l,n&&u.setAttribute("nonce",n),document.head.appendChild(u),d)return new Promise((m,g)=>{u.addEventListener("load",m),u.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(s){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=s,window.dispatchEvent(n),!n.defaultPrevented)throw s}return o.then(s=>{for(const n of s||[])n.status==="rejected"&&i(n.reason);return t().catch(i)})},Oe={registrar:e=>O.post("/asistencia/alumnos",e),listar:()=>O.get("/asistencia/alumnos"),obtenerPorId:e=>O.get(`/asistencia/alumnos/${e}`),porAlumno:e=>O.get(`/asistencia/alumnos/alumno/${e}`),porFecha:e=>O.get(`/asistencia/alumnos/fecha?fecha=${e}`),porAlumnoYFecha:(e,t)=>O.get(`/asistencia/alumnos/alumno/${e}/fecha?fecha=${t}`),porGradoYFecha:(e,t)=>O.get(`/asistencia/alumnos/grado/${encodeURIComponent(e)}/fecha?fecha=${t}`)},ae=()=>new Date().toISOString().split("T")[0],Qe=e=>e?new Date(e).toLocaleString("es-PE"):"—",Je=e=>e==="ENTRADA"?'<span class="badge badge-green">▶ ENTRADA</span>':'<span class="badge badge-red">◀ SALIDA</span>';let U=[],z=[],Ye=[],X="personal";async function Ct(e){var r,a,o,i,s,n;const t=b.hasRole("ADMINISTRADOR","PORTERO","SECRETARIA");e.innerHTML=`
    <div class="page-topbar">
      <div>
        <h1>Asistencia</h1>
    ...
    </div>`,e.querySelectorAll(".tab-btn").forEach(l=>{l.addEventListener("click",()=>{X=l.dataset.tab,e.querySelectorAll(".tab-btn").forEach(d=>{d.classList.toggle("active",d.dataset.tab===X),d.classList.toggle("btn-primary",d.dataset.tab===X),d.classList.toggle("btn-outline",d.dataset.tab!==X)}),e.querySelector("#section-personal").style.display=X==="personal"?"":"none",e.querySelector("#section-alumnos").style.display=X==="alumnos"?"":"none"})});try{Ye=await oe.listar()}catch{}await Ie(e,ae()),(r=e.querySelector("#btn-filter"))==null||r.addEventListener("click",()=>{const l=e.querySelector("#filter-fecha").value;l&&Ie(e,l)}),(a=e.querySelector("#btn-all"))==null||a.addEventListener("click",()=>Mt(e)),(o=e.querySelector("#btn-new"))==null||o.addEventListener("click",()=>Nt(e)),await Le(e,ae()),(i=e.querySelector("#btn-filter-alumnos"))==null||i.addEventListener("click",()=>{const l=e.querySelector("#filter-fecha-alumnos").value;l&&Le(e,l)}),(s=e.querySelector("#btn-all-alumnos"))==null||s.addEventListener("click",()=>Pt(e)),(n=e.querySelector("#btn-scan-qr"))==null||n.addEventListener("click",()=>Ht(e))}async function Ie(e,t){try{U=await Se.porFecha(t),e.querySelector("#total-personal").textContent=`${U.length}`,e.querySelector("#footer-personal").textContent=`${U.length} registros`,We(e,U)}catch{p("No se pudo conectar al servicio de asistencia (puerto 8081)","error")}}async function Mt(e){try{U=await Se.listar(),e.querySelector("#total-personal").textContent=`${U.length}`,e.querySelector("#footer-personal").textContent=`${U.length} registros totales`,We(e,U)}catch(t){p(t.message,"error")}}function We(e,t){const r=e.querySelector("#tbody-personal");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="5">Sin registros para este filtro</td></tr>';return}r.innerHTML=t.map(a=>`
    <tr>
      <td><strong>${a.nombrePersonal}</strong></td>
      <td><span class="badge badge-purple">${a.rolPersonal}</span></td>
      <td>${Je(a.tipoEvento)}</td>
      <td class="td-small">${Qe(a.horaEvento)}</td>
      <td class="td-muted">${a.observaciones??"—"}</td>
    </tr>`).join("")}function Nt(e){const t=Ye.map(a=>`<option value="${a.id}" data-nombre="${a.nombre}" data-rol="${a.rol}">${a.nombre} — ${a.rol}</option>`).join(""),r=new Date().toISOString().slice(0,16);N({title:"+ Registrar Asistencia — Personal",bodyHTML:`
      <div class="form-grid">
        <div class="form-group" style="grid-column:1/-1">
      ...
      </div>`,confirmText:"Registrar",onConfirm:async(a,o)=>{const i=a.querySelector("#m-usuario"),s=i.options[i.selectedIndex],n=i.value;if(!n){p("Selecciona un miembro del personal","warning");return}const l={usuarioId:Number(n),nombrePersonal:s.dataset.nombre,rolPersonal:s.dataset.rol,tipoEvento:a.querySelector("#m-tipo").value,horaEvento:a.querySelector("#m-hora").value?new Date(a.querySelector("#m-hora").value).toISOString():null,observaciones:a.querySelector("#m-obs").value.trim()||null};try{await Se.registrar(l),p("Asistencia registrada","success"),o();const d=e.querySelector("#filter-fecha").value||ae();await Ie(e,d)}catch(d){p(d.message,"error")}}})}async function Le(e,t){try{z=await Oe.porFecha(t),e.querySelector("#total-alumnos").textContent=`${z.length}`,e.querySelector("#footer-alumnos").textContent=`${z.length} registros`,Ke(e,z)}catch{p("No se pudo cargar asistencia de alumnos","error")}}async function Pt(e){try{z=await Oe.listar(),e.querySelector("#total-alumnos").textContent=`${z.length}`,e.querySelector("#footer-alumnos").textContent=`${z.length} registros totales`,Ke(e,z)}catch(t){p(t.message,"error")}}function Ke(e,t){const r=e.querySelector("#tbody-alumnos");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="6">Sin registros para este filtro</td></tr>';return}r.innerHTML=t.map(a=>`
    <tr>
      ...
      <td class="td-small">${Qe(a.horaEvento)}</td>
      <td class="td-muted">${a.observaciones??"—"}</td>
    </tr>`).join("")}function Ht(e){const t="qr-reader-"+Date.now();let r=null,a=!1;N({title:"📷 Escanear QR del Alumno",bodyHTML:`
  ...
  <div id="tab-horarios" class="tab-content" style="display:none">
    <div class="table-card">
  ...
  </div>`}async function ie(e=null){try{const[t,r]=await Promise.all([e?M.listarPorProfesor(e):M.listarTodos(),e?M.listarHorariosPorProfesor(e):M.listarHorarios()]);B=t||[],K=r||[]}catch(t){p(`Error cargando datos: ${t.message}`,"error"),B=[],K=[]}}function _(e){const t=e.querySelector("#cal-month-label"),r=e.querySelector("#cal-grid");if(!t||!r)return;t.textContent=`${Xe[q]} ${j}`;const a=new Date(j,q,1).getDay(),o=new Date(j,q+1,0).getDate(),i=new Date;let s=kt.map(n=>`<div class="cal-day-header">${n}</div>`).join("");for(let n=0;n<a;n++)s+='<div class="cal-cell cal-empty"></div>';for(let n=1;n<=o;n++){const l=new Date(j,q,n),d=Ee(l),c=n===i.getDate()&&q===i.getMonth()&&j===i.getFullYear(),u=$===d,m=B.filter(f=>{const E=new Date(f.fechaInicio);return E.getFullYear()===j&&E.getMonth()===q&&E.getDate()===n}),g=[...new Set(m.map(f=>f.tipoAgenda))].slice(0,3).map(f=>{var E;return`<span class="cal-dot" style="background:${((E=W[f])==null?void 0:E.bg)??"#888"}"></span>`}).join("");s+=`
      ...
      </div>`}r.innerHTML=s,r.querySelectorAll(".cal-cell[data-date]").forEach(n=>{n.addEventListener("click",()=>{$=n.dataset.date,_(e),J(e,$)})})}function J(e,t){const r=e.querySelector("#day-events-title"),a=e.querySelector("#day-events-list");if(!r||!a)return;const o=new Date(t+"T00:00:00");r.innerHTML=`<i class="fas fa-calendar-day"></i>
    Eventos del ${o.getDate()} de ${Xe[o.getMonth()]} ${o.getFullYear()}`;const i=B.filter(s=>{const n=new Date(s.fechaInicio);return Ee(n)===t});if(!i.length){a.innerHTML='<div class="day-empty"><i class="fas fa-calendar-xmark"></i><span>Sin eventos este día</span></div>';return}a.innerHTML=i.map(s=>{const n=W[s.tipoAgenda]??W.OTRO,l=me[s.estado]??me.ACTIVO;return`
      ...
      </tr>`}).join(""),e.querySelectorAll(".btn-edit-evento").forEach(s=>{s.addEventListener("click",()=>{const n=B.find(l=>l.id==s.dataset.id);n&&et(e,n)})}),e.querySelectorAll(".btn-del-evento").forEach(s=>{s.addEventListener("click",()=>st(e,s.dataset.id,"evento"))})}function Y(e,t=""){const r=e.querySelector("#horario-week-grid");if(!r)return;const a=["LUNES","MARTES","MIERCOLES","JUEVES","VIERNES","SABADO","DOMINGO"],o={LUNES:"Lunes",MARTES:"Martes",MIERCOLES:"Miércoles",JUEVES:"Jueves",VIERNES:"Viernes",SABADO:"Sábado",DOMINGO:"Domingo"},i=b.hasRole("ADMINISTRADOR","DIRECTOR"),s=t?[t]:a,n=K;if(!n.length){r.innerHTML=`<div class="day-empty" style="padding:40px;text-align:center">
      ...
      </div>`:""}).join(""),e.querySelectorAll(".btn-edit-horario").forEach(l=>{l.addEventListener("click",()=>{const d=K.find(c=>c.id==l.dataset.id);d&&tt(e,d)})}),e.querySelectorAll(".btn-del-horario").forEach(l=>{l.addEventListener("click",()=>st(e,l.dataset.id,"horario"))})}function jt(e){e.addEventListener("click",t=>{const r=t.target.closest(".agenda-tab");r&&(Ae=r.dataset.tab,e.querySelectorAll(".agenda-tab").forEach(a=>a.classList.toggle("active",a===r)),e.querySelectorAll(".tab-content").forEach(a=>{a.style.display=a.id===`tab-${Ae}`?"":"none"}),Ae==="horarios"&&Y(e))})}function Ut(e,t,r){var a,o,i,s,n,l,d,c;(a=e.querySelector("#cal-prev"))==null||a.addEventListener("click",()=>{q===0?(q=11,j--):q--,_(e),$&&J(e,$)}),(o=e.querySelector("#cal-next"))==null||o.addEventListener("click",()=>{q===11?(q=0,j++):q++,_(e),$&&J(e,$)}),(i=e.querySelector("#btn-filter-prof"))==null||i.addEventListener("click",async()=>{var m,g;const u=(g=(m=e.querySelector("#filter-profesor"))==null?void 0:m.value)==null?void 0:g.trim();await ie(u?Number(u):null),_(e),se(e),Y(e),$&&J(e,$)}),(s=e.querySelector("#btn-clear-filter"))==null||s.addEventListener("click",async()=>{const u=e.querySelector("#filter-profesor");u&&(u.value=""),await ie(null),_(e),se(e),Y(e),$&&J(e,$)}),(n=e.querySelector("#search-eventos"))==null||n.addEventListener("input",u=>{se(e,u.target.value)}),(l=e.querySelector("#filter-dia"))==null||l.addEventListener("change",u=>{Y(e,u.target.value)}),(d=e.querySelector("#btn-new-evento"))==null||d.addEventListener("click",()=>{et(e,null)}),(c=e.querySelector("#btn-new-horario"))==null||c.addEventListener("click",()=>{tt(e,null)}),se(e)}function et(e,t){const r=!!t,a=r?"Editar Evento de Agenda":"Nuevo Evento de Agenda",o=b.hasRole("PROFESOR"),i=(t==null?void 0:t.profesorId)??(o?b.id():""),s=(t==null?void 0:t.nombreProfesor)??(o?b.nombre():""),n=d=>d?d.replace("T","T").substring(0,16):"",l=`
    ...
    </div>`;N({title:a,bodyHTML:l,onConfirm:async(d,c)=>{const u={profesorId:Number(document.getElementById("ev-profesorId").value),nombreProfesor:document.getElementById("ev-nombreProfesor").value.trim(),especialidad:document.getElementById("ev-especialidad").value.trim()||null,titulo:document.getElementById("ev-titulo").value.trim(),descripcion:document.getElementById("ev-descripcion").value.trim()||null,fechaInicio:document.getElementById("ev-inicio").value,fechaFin:document.getElementById("ev-fin").value,tipoAgenda:document.getElementById("ev-tipo").value,estado:document.getElementById("ev-estado").value,lugar:document.getElementById("ev-lugar").value.trim()||null,recurrente:document.getElementById("ev-recurrente").checked,diaSemana:document.getElementById("ev-recurrente").checked?document.getElementById("ev-diaSemana").value:null};if(!u.profesorId||!u.nombreProfesor||!u.titulo||!u.fechaInicio||!u.fechaFin){p("Completa los campos obligatorios (*)","error");return}try{r?(await M.actualizar(t.id,u),p("Evento actualizado correctamente","success")):(await M.crear(u),p("Evento creado correctamente","success"));const m=b.hasRole("PROFESOR")?b.id():null;await ie(m),_(e),se(e),$&&J(e,$),c()}catch(m){p(`Error: ${m.message}`,"error")}}}),setTimeout(()=>{var f,E,re,F;const d=R=>{if(!R)return"";const[w,y,A]=R.split("-").map(Number),T=new Date(w,y-1,A).getDay();return["DOMINGO","LUNES","MARTES","MIERCOLES","JUEVES","VIERNES","SABADO"][T]},c=R=>{const y={DOMINGO:0,LUNES:1,MARTES:2,MIERCOLES:3,JUEVES:4,VIERNES:5,SABADO:6}[R];if(y===void 0)return null;const A=new Date,P=A.getDay();let T=y-P;T<0&&(T+=7);const I=new Date(A.getFullYear(),A.getMonth(),A.getDate()+T),x=I.getFullYear(),V=String(I.getMonth()+1).padStart(2,"0"),C=String(I.getDate()).padStart(2,"0");return`${x}-${V}-${C}`},u=(R=!1)=>{const w=document.getElementById("ev-horario-select");if(!w)return;const y=w.options[w.selectedIndex],A=document.getElementById("ev-fecha"),P=document.getElementById("ev-inicio"),T=document.getElementById("ev-fin");if(y&&y.dataset.dia){const x=y.dataset.dia;if(!R&&A){const V=c(x);V&&(A.value=V)}}const I=A==null?void 0:A.value;if(I&&y&&y.dataset.inicio&&y.dataset.fin){const x=y.dataset.inicio,V=y.dataset.fin;P&&(P.value=`${I}T${x}`),T&&(T.value=`${I}T${V}`);const C=y.dataset.dia,G=document.getElementById("ev-diaSemana");C&&G&&(G.value=C);const v=y.dataset.materia,H=document.getElementById("ev-especialidad");v&&H&&(H.value=v);const L=document.getElementById("ev-fecha-warning");if(C){const D=d(I);if(D!==C){const ne={LUNES:"Lunes",MARTES:"Martes",MIERCOLES:"Miércoles",JUEVES:"Jueves",VIERNES:"Viernes",SABADO:"Sábado",DOMINGO:"Domingo"},$e=ne[C]||C,Re=ne[D]||D;L&&(L.innerHTML=`<i class="fas fa-exclamation-triangle"></i> La fecha seleccionada es <strong>${Re}</strong>, pero el horario seleccionado es para los <strong>${$e}</strong>.`,L.style.display="block")}else L&&(L.style.display="none",L.innerHTML="")}else L&&(L.style.display="none",L.innerHTML="")}else{P&&(P.value=""),T&&(T.value="");const x=document.getElementById("ev-fecha-warning");x&&(x.style.display="none",x.innerHTML="")}},m=async R=>{var y,A,P;const w=document.getElementById("ev-horario-select");if(w){w.innerHTML='<option value="">Cargando horarios...</option>';try{const I=(await M.listarHorariosPorProfesor(R)||[]).filter(v=>v.activo),x=((y=B.find(v=>v.profesorId==R&&v.especialidad))==null?void 0:y.especialidad)||((A=I.find(v=>v.materia))==null?void 0:A.materia)||((P=K.find(v=>v.profesorId==R&&v.materia))==null?void 0:P.materia)||"",V=document.getElementById("ev-especialidad");if(V&&x&&(V.value=x),I.length===0){w.innerHTML='<option value="">El profesor no tiene horarios registrados</option>',u(!0);return}const C={LUNES:"Lunes",MARTES:"Martes",MIERCOLES:"Miércoles",JUEVES:"Jueves",VIERNES:"Viernes",SABADO:"Sábado",DOMINGO:"Domingo"};let G='<option value="">Seleccione un horario...</option>';if(I.forEach(v=>{const H=v.horaInicio.slice(0,5),L=v.horaFin.slice(0,5),D=C[v.diaSemana]||v.diaSemana;let ne=!1;if(t){const $e=n(t.fechaInicio).split("T")[1],Re=n(t.fechaFin).split("T")[1];$e===H&&Re===L&&v.diaSemana===t.diaSemana&&(ne=!0)}G+=`<option value="${v.id}" data-inicio="${H}" data-fin="${L}" data-dia="${v.diaSemana}" data-materia="${v.materia}" ${ne?"selected":""}>
            ${D}: ${H} - ${L} (${v.materia}${v.aula?" · "+v.aula:""})
          </option>`}),t){const v=n(t.fechaInicio).split("T")[1],H=n(t.fechaFin).split("T")[1];if(!I.some(D=>D.horaInicio.slice(0,5)===v&&D.horaFin.slice(0,5)===H&&D.diaSemana===t.diaSemana)){const D=C[t.diaSemana]||t.diaSemana||"";G=`<option value="original" data-inicio="${v}" data-fin="${H}" data-dia="${t.diaSemana}" selected>
              [Horario Original] ${D?D+": ":""}${v} - ${H}
            </option>`+G}}w.innerHTML=G,u(!0)}catch(T){w.innerHTML='<option value="">Error al cargar horarios</option>',console.error(T)}}};at("ev-profesor-search","ev-profesor-dropdown","ev-profesorId","ev-nombreProfesor",async R=>{if(R)await m(R);else{const w=document.getElementById("ev-horario-select");w&&(w.innerHTML='<option value="">Seleccione un profesor primero...</option>'),u(!0)}}),(f=document.getElementById("ev-recurrente"))==null||f.addEventListener("change",R=>{document.getElementById("dia-semana-group").style.display=R.target.checked?"":"none"}),(E=document.getElementById("ev-fecha"))==null||E.addEventListener("change",()=>u(!0)),(re=document.getElementById("ev-horario-select"))==null||re.addEventListener("change",()=>u(!1));const g=(F=document.getElementById("ev-profesorId"))==null?void 0:F.value;g&&m(g)},50)}function tt(e,t){var l,d;const r=!!t,a=r?"Editar Horario Semanal":"Nuevo Horario Semanal",o=b.hasRole("PROFESOR"),i=(t==null?void 0:t.profesorId)??(o?b.id():""),s=(t==null?void 0:t.nombreProfesor)??(o?b.nombre():""),n=`
    <div class="form-grid-2">
      <div class="form-group" style="grid-column:1/-1">
        <label>Profesor *</label>
      ...
      </div>`).join(""),s.classList.add("open"),s.querySelectorAll(".profesor-option").forEach(f=>{f.addEventListener("mousedown",E=>{E.preventDefault(),n.value=f.dataset.id,l.value=f.dataset.nombre,i.value=f.dataset.nombre,i.classList.add("profesor-selected"),s.classList.remove("open"),s.innerHTML="",typeof o=="function"&&o(f.dataset.id)})})}i.addEventListener("input",()=>{n.value="",l.value="",i.classList.remove("profesor-selected"),c(i.value),typeof o=="function"&&o(null)}),i.addEventListener("focus",()=>{c(i.value)}),i.addEventListener("blur",()=>{setTimeout(()=>{if(s.classList.remove("open"),s.innerHTML="",!n.value&&i.value.trim()){const u=d.find(m=>m.nombre.toLowerCase()===i.value.trim().toLowerCase());u&&(n.value=u.id,l.value=u.nombre,i.classList.add("profesor-selected"),typeof o=="function"&&o(u.id))}},200)})}function st(e,t,r){N({title:r==="evento"?"Eliminar Evento":"Eliminar Horario",bodyHTML:`<p style="color:var(--text2);line-height:1.6">
    ¿Estás seguro que deseas eliminar este ${r==="evento"?"evento de agenda":"bloque de horario"}?
    <strong>Esta acción no se puede deshacer.</strong>
  </p>`,onConfirm:async(i,s)=>{try{r==="evento"?(await M.eliminar(t),p("Evento eliminado","success")):(await M.eliminarHorario(t),p("Horario eliminado","success"));const n=b.hasRole("PROFESOR")?b.id():null;await ie(n),_(e),se(e),Y(e),$&&J(e,$),s()}catch(n){p(`Error: ${n.message}`,"error")}}})}const zt={listar:()=>h.get("/auditoria")};let ce=[];const _t={ADMINISTRADOR:"badge-red",PORTERO:"badge-blue",SECRETARIA:"badge-green",DIRECTOR:"badge-yellow",PROFESOR:"badge-purple",SISTEMA:"badge-gray"},Gt=e=>`<span class="badge ${_t[e]??"badge-gray"}">${e}</span>`;function Qt(e){return e?new Date(e).toLocaleString("es-PE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"}):""}async function Jt(e){e.innerHTML=`
    <div class="page-topbar">
      <div>
    ...
    </div>`,await Yt(e);const t=e.querySelector("#aud-search"),r=e.querySelector("#aud-filter-rol"),a=()=>{const o=t.value.toLowerCase().trim(),i=r.value;let s=ce;i&&(s=s.filter(n=>n.rol===i)),o&&(s=s.filter(n=>{var l,d,c;return((l=n.usuario)==null?void 0:l.toLowerCase().includes(o))||((d=n.accion)==null?void 0:d.toLowerCase().includes(o))||((c=n.detalles)==null?void 0:c.toLowerCase().includes(o))})),ot(e,s)};t.addEventListener("input",a),r.addEventListener("change",a)}async function Yt(e){try{ce=await zt.listar(),e.querySelector("#aud-total").textContent=`${ce.length}`,e.querySelector("#aud-footer-lbl").textContent=`${ce.length} transacciones registradas`,ot(e,ce)}catch(t){p(t.message,"error")}}function ot(e,t){const r=e.querySelector("#aud-tbody");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="5">Sin transacciones registradas que coincidan</td></tr>';return}r.innerHTML=t.map(a=>`
    <tr>
      <td class="td-muted">${Qt(a.fecha)}</td>
    ...
    </tr>`).join("")}const de=document.getElementById("app"),Wt={"#/login":{page:bt,auth:!1},"#/dashboard":{page:gt,auth:!0},"#/visitas":{page:$t,auth:!0},"#/visitantes":{page:Rt,auth:!0},"#/alumnos":{page:wt,auth:!0},"#/usuarios":{page:Ot,auth:!0,roles:["ADMINISTRADOR"]},"#/auditoria":{page:Jt,auth:!0,roles:["ADMINISTRADOR"]},"#/asistencia":{page:Ct,auth:!0},"#/agenda":{page:Bt,auth:!0}};async function it(){const e=window.location.hash||"#/dashboard",t=Wt[e];if(!t){window.location.hash="#/dashboard";return}if(!t.auth){if(b.isLogged()){window.location.hash="#/dashboard";return}de.innerHTML="",t.page(de);return}if(!b.isLogged()){window.location.hash="#/login";return}if(t.roles&&!t.roles.includes(b.rol())){window.location.hash="#/dashboard";return}de.innerHTML=`
    <div class="app-container">
      <div id="sidebar-wrap"></div>
      <div class="main-content" id="page-root"></div>
    </div>`,lt(de.querySelector("#sidebar-wrap"));const r=de.querySelector("#page-root");try{await t.page(r)}catch(a){r.innerHTML=`
