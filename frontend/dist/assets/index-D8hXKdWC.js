...
<div class="svc-row">
    ...
    </div>`,await Ne(e),(t=e.querySelector("#search"))==null||t.addEventListener("input",a=>{const i=a.target.value.toLowerCase();Pe(e,ue.filter(r=>{var s,n,l;return((s=r.nombreVisitante)==null?void 0:s.toLowerCase().includes(i))||((n=r.dniVisitante)==null?void 0:n.toLowerCase().includes(i))||((l=r.motivo)==null?void 0:l.toLowerCase().includes(i))}))}),(o=e.querySelector("#btn-new"))==null||o.addEventListener("click",()=>He(e))}async function Ne(e){try{ue=await Z.listar(),e.querySelector("#footer-count").textContent=`${ue.length} registros`,Pe(e,ue)}catch(t){p(t.message,"error")}}function Pe(e,t){const o=e.querySelector("#tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="7">No hay registros</td></tr>';return}o.innerHTML=t.map(a=>{var i;return`
    <tr>
    ...
    </tr>`}).join(""),fe()&&o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>{He(e,ue.find(i=>i.id==a.dataset.id))}))}async function He(e,t=null){const o=!!t;let a=[];try{a=await Z.buscarUsuarios("")}catch{}const i=new Date().toISOString().slice(0,16);N({title:o?"✏️ Editar Visita":"+ Nueva Visita",bodyHTML:`
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select class="form-control" id="m-estado">
            ${ht.map(r=>`<option value="${r}" ${((t==null?void 0:t.estadoRegistro)??"REGISTRADO")===r?"selected":""}>${r}</option>`).join("")}
          </select>
        </div>
        <div class="form-group">
    ...
    </tr>`}).join(""),ge()&&o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>Be(e,ee.find(i=>i.id==a.dataset.id))))}function Be(e,t=null){N({title:t?"✏️ Editar Visitante":"+ Nuevo Visitante",bodyHTML:`
      <div class="form-grid">
        <div class="form-group">
          <label>DNI *</label>
    ...
    </tr>`}).join(""),he()&&o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>ze(e,te.find(i=>i.id==a.dataset.id)))),o.querySelectorAll(".qr-btn").forEach(a=>a.addEventListener("click",()=>At(a.dataset.id,a.dataset.nombre)))}async function At(e,t){let o=null;N({title:`📱 Código QR — ${t}`,bodyHTML:`
    ...
    </div>`,await Te(e),e.querySelector("#search").addEventListener("input",t=>{const o=t.target.value.toLowerCase();_e(e,Q.filter(a=>{var i,r;return((i=a.nombre)==null?void 0:i.toLowerCase().includes(o))||((r=a.usuario)==null?void 0:r.toLowerCase().includes(o))}))}),e.querySelector("#btn-new").addEventListener("click",()=>Ge(e))}async function Te(e){try{Q=await oe.listar(),e.querySelector("#total").textContent=`${Q.length}`,e.querySelector("#footer-lbl").textContent=`${Q.length} usuarios`,_e(e,Q)}catch(t){p(t.message,"error")}}function _e(e,t){const o=e.querySelector("#tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="4">Sin usuarios registrados</td></tr>';return}o.innerHTML=t.map(a=>`
    <tr>
      <td><strong>${a.nombre}</strong></td>
    ...
    </tr>`).join(""),o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>Ge(e,Q.find(i=>i.id==a.dataset.id)))),o.querySelectorAll(".del-btn").forEach(a=>a.addEventListener("click",()=>Ot(e,Q.find(i=>i.id==a.dataset.id))))}function Ge(e,t=null){const o=It.map(a=>`<option value="${a}" ${(t==null?void 0:t.rol)===a?"selected":""}>${a}</option>`).join("");N({title:t?"✏️ Editar Usuario":"+ Nuevo Usuario",bodyHTML:`
      <div class="form-grid">
        <div class="form-group">
          <label>Nombre completo *</label>
      ...
      </div>`,confirmText:t?"Actualizar":"Crear Usuario",onConfirm:async(a,i)=>{const r={nombre:a.querySelector("#m-nombre").value.trim(),usuario:a.querySelector("#m-usuario").value.trim(),contraseña:a.querySelector("#m-pass").value,rol:a.querySelector("#m-rol").value};if(!r.nombre||!r.usuario||!r.rol){p("Nombre, usuario y rol son obligatorios","warning");return}if(!t&&!r.contraseña){p("La contraseña es obligatoria","warning");return}try{t?await oe.actualizar(t.id,r):await oe.registrar(r),p(t?"Usuario actualizado":"Usuario creado","success"),i(),await Te(e)}catch(s){p(s.message,"error")}}})}function Ot(e,t){N({title:"🗑 Eliminar Usuario",bodyHTML:`<p style="color:var(--text2);line-height:1.7">¿Estás seguro de eliminar a <strong style="color:var(--text)">${t.nombre}</strong> (@${t.usuario})?<br/>Esta acción no se puede deshacer.</p>`,confirmText:"Eliminar",danger:!0,onConfirm:async(o,a)=>{try{await oe.eliminar(t.id),p("Usuario eliminado","success"),a(),await Te(e)}catch(i){p(i.message,"error")}}})}const Dt="modulepreload",qt=function(e){return"/"+e},qe={},Ce=function(t,o,a){let i=Promise.resolve();if(o&&o.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),n=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));i=Promise.allSettled(o.map(l=>{if(l=qt(l),l in qe)return;qe[l]=!0;const d=l.endsWith(".css"),c=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${c}`))return;const u=document.createElement("link");if(u.rel=d?"stylesheet":Dt,d||(u.as="script"),u.crossOrigin="",u.href=l,n&&u.setAttribute("nonce",n),document.head.appendChild(u),d)return new Promise((m,g)=>{u.addEventListener("load",m),u.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${l}`)))})}))}function r(s){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=s,window.dispatchEvent(n),!n.defaultPrevented)throw s}return i.then(s=>{for(const n of s||[])n.status==="rejected"&&r(n.reason);return t().catch(r)})},xe={registrar:e=>x.post("/asistencia/alumnos",e),listar:()=>x.get("/asistencia/alumnos"),obtenerPorId:e=>x.get(`/asistencia/alumnos/${e}`),porAlumno:e=>x.get(`/asistencia/alumnos/alumno/${e}`),porFecha:e=>x.get(`/asistencia/alumnos/fecha?fecha=${e}`),porAlumnoYFecha:(e,t)=>x.get(`/asistencia/alumnos/alumno/${e}/fecha?fecha=${t}`),porGradoYFecha:(e,t)=>x.get(`/asistencia/alumnos/grado/${encodeURIComponent(e)}/fecha?fecha=${t}`)},ae=()=>new Date().toISOString().split("T")[0],Qe=e=>e?new Date(e).toLocaleString("es-PE"):"—",Je=e=>e==="ENTRADA"?'<span class="badge badge-green">▶ ENTRADA</span>':'<span class="badge badge-red">◀ SALIDA</span>';let U=[],z=[],Ye=[],X="personal";async function Ct(e){var o,a,i,r,s,n;const t=b.hasRole("ADMINISTRADOR","PORTERO","SECRETARIA");e.innerHTML=`
    <div class="page-topbar">
      <div>
        <h1>Asistencia</h1>
    ...
    </tr>`).join("")}function Nt(e){const t=Ye.map(a=>`<option value="${a.id}" data-nombre="${a.nombre}" data-rol="${a.rol}">${a.nombre} — ${a.rol}</option>`).join(""),o=new Date().toISOString().slice(0,16);N({title:"+ Registrar Asistencia — Personal",bodyHTML:`
      <div class="form-grid">
        <div class="form-group" style="grid-column:1/-1">
    ...
    <tr>
      ...
      <td class="td-small">${Qe(a.horaEvento)}</td>
      <td class="td-muted">${a.observaciones??"—"}</td>
    </tr>`).join("")}function Ht(e){const t="qr-reader-"+Date.now();let o=null,a=!1;N({title:"📷 Escanear QR del Alumno",bodyHTML:`
  ...
  <div id="tab-horarios" class="tab-content" style="display:none">
    ...
    </div>`,await Yt(e);const t=e.querySelector("#aud-search"),o=e.querySelector("#aud-filter-rol"),a=()=>{const i=t.value.toLowerCase().trim(),r=o.value;let s=ce;r&&(s=s.filter(n=>n.rol===r)),i&&(s=s.filter(n=>{var l,d,c;return((l=n.usuario)==null?void 0:l.toLowerCase().includes(i))||((d=n.accion)==null?void 0:d.toLowerCase().includes(i))||((c=n.detalles)==null?void 0:c.toLowerCase().includes(i))})),ot(e,s)};t.addEventListener("input",a),o.addEventListener("change",a)}async function Yt(e){try{ce=await zt.listar(),e.querySelector("#aud-total").textContent=`${ce.length}`,e.querySelector("#aud-footer-lbl").textContent=`${ce.length} transacciones registradas`,ot(e,ce)}catch(t){p(t.message,"error")}}function ot(e,t){const o=e.querySelector("#aud-tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="5">Sin transacciones registradas que coincidan</td></tr>';return}o.innerHTML=t.map(a=>`
    <tr>
