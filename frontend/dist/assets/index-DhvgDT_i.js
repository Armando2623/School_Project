...
<div class="svc-row">
    ...
    </div>`;const t=m.token(),r=t?{Authorization:`Bearer ${t}`}:{},a=async(o,i,s={})=>{const n=e.querySelector(`#${i}`);try{const l=await fetch(o,{signal:AbortSignal.timeout(3e3),headers:s});n.classList.add(l.status<500?"online":"offline")}catch{n.classList.add("offline")}};a(`${Me}/api/alumnos`,"dot-mvc",r),a(`${Ne}/api/asistencia`,"dot-ast",r);try{const[o,i]=await Promise.all([ae.listar(),ge.listar()]),s=o.filter(c=>{var u;return(u=c.horaIngreso)==null?void 0:u.startsWith(Ve())}),n=o.filter(c=>c.estadoRegistro==="EN_CURSO");e.querySelector("#s-activos").textContent=n.length,e.querySelector("#s-hoy").textContent=s.length,e.querySelector("#s-alumnos").textContent=i.length,e.querySelector("#footer-count").textContent=`${s.length} hoy`;const l=e.querySelector("#recent-tbody"),d=[...o].reverse().slice(0,5);d.length?l.innerHTML=d.map(c=>`
        <tr>
    ...
    </div>`,e.querySelectorAll(".tab-btn").forEach(l=>{l.addEventListener("click",()=>{ee=l.dataset.tab,e.querySelectorAll(".tab-btn").forEach(d=>{d.classList.toggle("active",d.dataset.tab===ee),d.classList.toggle("btn-primary",d.dataset.tab===ee),d.classList.toggle("btn-outline",d.dataset.tab!==ee)}),e.querySelector("#section-personal").style.display=ee==="personal"?"":"none",e.querySelector("#section-alumnos").style.display=ee==="alumnos"?"":"none"})});try{ot=await ne.listar()}catch{}await De(e,ie()),(r=e.querySelector("#btn-filter"))==null||r.addEventListener("click",()=>{const l=e.querySelector("#filter-fecha").value;l&&De(e,l)}),(a=e.querySelector("#btn-all"))==null||a.addEventListener("click",()=>_t(e)),(o=e.querySelector("#btn-new"))==null||o.addEventListener("click",()=>Gt(e)),await qe(e,ie()),(i=e.querySelector("#btn-filter-alumnos"))==null||i.addEventListener("click",()=>{const l=e.querySelector("#filter-fecha-alumnos").value;l&&qe(e,l)}),(s=e.querySelector("#btn-all-alumnos"))==null||s.addEventListener("click",()=>Qt(e)),(n=e.querySelector("#btn-scan-qr"))==null||n.addEventListener("click",()=>Jt(e))}async function De(e,t){try{z=await Re.porFecha(t),e.querySelector("#total-personal").textContent=`${z.length}`,e.querySelector("#footer-personal").textContent=`${z.length} registros`,it(e,z)}catch{p("No se pudo conectar al servicio de asistencia (puerto 8081)","error")}}async function _t(e){try{z=await Re.listar(),e.querySelector("#total-personal").textContent=`${z.length}`,e.querySelector("#footer-personal").textContent=`${z.length} registros totales`,it(e,z)}catch(t){p(t.message,"error")}}function it(e,t){const r=e.querySelector("#tbody-personal");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="5">Sin registros para este filtro</td></tr>';return}r.innerHTML=t.map(a=>`
    <tr>
      ...
      </div>`,confirmText:"Registrar",onConfirm:async(a,o)=>{const i=a.querySelector("#m-usuario"),s=i.options[i.selectedIndex],n=i.value;if(!n){p("Selecciona un miembro del personal","warning");return}const l={usuarioId:Number(n),nombrePersonal:s.dataset.nombre,rolPersonal:s.dataset.rol,tipoEvento:a.querySelector("#m-tipo").value,horaEvento:a.querySelector("#m-hora").value?new Date(a.querySelector("#m-hora").value).toISOString():null,observaciones:a.querySelector("#m-obs").value.trim()||null};try{await Re.registrar(l),p("Asistencia registrada","success"),o();const d=e.querySelector("#filter-fecha").value||ie();await De(e,d)}catch(d){p(d.message,"error")}}})}async function qe(e,t){try{U=await ke.porFecha(t),e.querySelector("#total-alumnos").textContent=`${U.length}`,e.querySelector("#footer-alumnos").textContent=`${U.length} registros`,rt(e,U)}catch{p("No se pudo cargar asistencia de alumnos","error")}}async function Qt(e){try{U=await ke.listar(),e.querySelector("#total-alumnos").textContent=`${U.length}`,e.querySelector("#footer-alumnos").textContent=`${U.length} registros totales`,rt(e,U)}catch(t){p(t.message,"error")}}function rt(e,t){const r=e.querySelector("#tbody-alumnos");if(!t.length){r.innerHTML='<tr class="empty-row"><td colspan="6">Sin registros para este filtro</td></tr>';return}r.innerHTML=t.map(a=>`
    <tr>
