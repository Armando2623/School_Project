(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function o(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(i){if(i.ep)return;i.ep=!0;const r=o(i);fetch(i.href,r)}})();const we="sg_session",b={save(e){sessionStorage.setItem(we,JSON.stringify(e))},get(){try{return JSON.parse(sessionStorage.getItem(we))}catch{return null}},clear(){sessionStorage.removeItem(we)},token(){var e;return((e=this.get())==null?void 0:e.token)??null},usuario(){var e;return((e=this.get())==null?void 0:e.usuario)??null},nombre(){var e;return((e=this.get())==null?void 0:e.nombre)??null},rol(){var e;return((e=this.get())==null?void 0:e.rol)??null},id(){var e;return((e=this.get())==null?void 0:e.id)??null},isLogged(){return!!this.token()},hasRole(...e){return e.includes(this.rol())}},nt=[{section:"PRINCIPAL",items:[{icon:"fas fa-chart-line",label:"Dashboard",hash:"#/dashboard",roles:null},{icon:"fas fa-clipboard-list",label:"Visitas",hash:"#/visitas",roles:["ADMINISTRADOR","PORTERO","SECRETARIA","DIRECTOR","PROFESOR"]}]},{section:"GESTIÓN",items:[{icon:"fas fa-users",label:"Visitantes",hash:"#/visitantes",roles:["ADMINISTRADOR","SECRETARIA"]},{icon:"fas fa-user-graduate",label:"Alumnos",hash:"#/alumnos",roles:["ADMINISTRADOR","SECRETARIA","PROFESOR"]},{icon:"fas fa-check-square",label:"Asistencia",hash:"#/asistencia",roles:["ADMINISTRADOR","DIRECTOR","SECRETARIA","PORTERO","PROFESOR"]},{icon:"fas fa-calendar-alt",label:"Agenda",hash:"#/agenda",roles:["ADMINISTRADOR","DIRECTOR","SECRETARIA","PORTERO","PROFESOR"]},{icon:"fas fa-user-cog",label:"Usuarios",hash:"#/usuarios",roles:["ADMINISTRADOR"]},{icon:"fas fa-history",label:"Auditoría",hash:"#/auditoria",roles:["ADMINISTRADOR"]}]}],lt={ADMINISTRADOR:"Administrador",PORTERO:"Portero",SECRETARIA:"Secretaria",DIRECTOR:"Director",PROFESOR:"Profesor"};function dt(e){const t=b.rol(),o=window.location.hash||"#/dashboard",a=nt.map(i=>{const r=i.items.filter(s=>!s.roles||s.roles.includes(t));return r.length?`
...
<div class="svc-row">
    ...
    </div>`,l.classList.add("active"),document.body.appendChild(l);const d=()=>{n&&n(),l.remove()};return l.querySelector("#modal-close-btn").addEventListener("click",d),(c=l.querySelector("#modal-cancel-btn"))==null||c.addEventListener("click",d),l.querySelector("#modal-confirm-btn").addEventListener("click",()=>{o?o(l,d):d()}),l.addEventListener("click",u=>{u.target===l&&d()}),s&&setTimeout(()=>s(l),0),l}const vt=["REGISTRADO","EN_CURSO","COMPLETADO"],gt={REGISTRADO:"badge-blue",EN_CURSO:"badge-yellow",COMPLETADO:"badge-green"},ht=e=>`<span class="badge ${gt[e]??"badge-gray"}">${e??"—"}</span>`,yt=e=>e?new Date(e).toLocaleString("es-PE"):"—",fe=()=>b.hasRole("ADMINISTRADOR","PORTERO","SECRETARIA");let ue=[];async function Et(e){var t,o;e.innerHTML=`
    <div class="page-topbar">
      <div>
    ...
    </div>`,await Pe(e),(t=e.querySelector("#search"))==null||t.addEventListener("input",a=>{const i=a.target.value.toLowerCase();He(e,ue.filter(r=>{var s,n,l;return((s=r.nombreVisitante)==null?void 0:s.toLowerCase().includes(i))||((n=r.dniVisitante)==null?void 0:n.toLowerCase().includes(i))||((l=r.motivo)==null?void 0:l.toLowerCase().includes(i))}))}),(o=e.querySelector("#btn-new"))==null||o.addEventListener("click",()=>ke(e))}async function Pe(e){try{ue=await Z.listar(),e.querySelector("#footer-count").textContent=`${ue.length} registros`,He(e,ue)}catch(t){p(t.message,"error")}}function He(e,t){const o=e.querySelector("#tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="7">No hay registros</td></tr>';return}o.innerHTML=t.map(a=>{var i;return`
    <tr>
      <td><strong>${a.nombreVisitante}</strong></td>
      <td class="td-muted">${a.dniVisitante}</td>
      <td>${a.motivo}</td>
      <td>${((i=a.usuario)==null?void 0:i.nombre)??"—"}</td>
      <td class="td-small">${yt(a.horaIngreso)}</td>
      <td>${ht(a.estadoRegistro)}</td>
      ${fe()?`<td><button class="btn btn-outline btn-sm edit-btn" data-id="${a.id}">✏️ Editar</button></td>`:""}
    </tr>`}).join(""),fe()&&o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>{ke(e,ue.find(i=>i.id==a.dataset.id))}))}async function ke(e,t=null){const o=!!t;let a=[];try{a=await Z.buscarUsuarios("")}catch{}const i=new Date().toISOString().slice(0,16);N({title:o?"✏️ Editar Visita":"+ Nueva Visita",bodyHTML:`
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select class="form-control" id="m-estado">
            ${vt.map(r=>`<option value="${r}" ${((t==null?void 0:t.estadoRegistro)??"REGISTRADO")===r?"selected":""}>${r}</option>`).join("")}
          </select>
        </div>
        <div class="form-group">
    ...
    </div>`,await Ve(e),e.querySelector("#search").addEventListener("input",o=>{const a=o.target.value.toLowerCase();Be(e,ee.filter(i=>{var r,s;return((r=i.dniVisitante)==null?void 0:r.toLowerCase().includes(a))||((s=i.nombreVisitante)==null?void 0:s.toLowerCase().includes(a))}))}),(t=e.querySelector("#btn-new"))==null||t.addEventListener("click",()=>Fe(e))}async function Ve(e){try{ee=await ve.listar(),e.querySelector("#total").textContent=`${ee.length}`,e.querySelector("#footer-lbl").textContent=`${ee.length} visitantes registrados`,Be(e,ee)}catch(t){p(t.message,"error")}}function Be(e,t){const o=e.querySelector("#tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="6">Sin visitantes registrados</td></tr>';return}o.innerHTML=t.map(a=>{var i;return`
    <tr>
    ...
    </tr>`}).join(""),ge()&&o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>Fe(e,ee.find(i=>i.id==a.dataset.id))))}function Fe(e,t=null){N({title:t?"✏️ Editar Visitante":"+ Nuevo Visitante",bodyHTML:`
      <div class="form-grid">
        <div class="form-group">
          <label>DNI *</label>
    ...
    </div>`,await Ue(e),e.querySelector("#search").addEventListener("input",o=>{const a=o.target.value.toLowerCase();ze(e,te.filter(i=>{var r,s,n;return((r=i.nombre)==null?void 0:r.toLowerCase().includes(a))||((s=i.grado)==null?void 0:s.toLowerCase().includes(a))||((n=i.seccion)==null?void 0:n.toLowerCase().includes(a))}))}),(t=e.querySelector("#btn-new"))==null||t.addEventListener("click",()=>_e(e))}async function Ue(e){try{[te,je]=await Promise.all([pe.listar(),ve.listar()]),e.querySelector("#total").textContent=`${te.length}`,e.querySelector("#footer-lbl").textContent=`${te.length} alumnos registrados`,ze(e,te)}catch(t){p(t.message,"error")}}function ze(e,t){const o=e.querySelector("#tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="6">Sin alumnos registrados</td></tr>';return}o.innerHTML=t.map(a=>{var s,n;const r=!!((s=a.apoderado)!=null&&s.email)?`<span class="badge badge-green" title="El apoderado ${a.apoderado.nombreVisitante} recibirá notificaciones en ${a.apoderado.email}" style="cursor:default">📧 Email</span>`:'<span class="td-muted" title="El apoderado no tiene email registrado">—</span>';return`
    <tr>
    ...
    </tr>`}).join(""),he()&&o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>_e(e,te.find(i=>i.id==a.dataset.id)))),o.querySelectorAll(".qr-btn").forEach(a=>a.addEventListener("click",()=>Rt(a.dataset.id,a.dataset.nombre)))}async function Rt(e,t){let o=null;N({title:`📱 Código QR — ${t}`,bodyHTML:`
    ...
    </div>`,await xe(e),e.querySelector("#search").addEventListener("input",t=>{const o=t.target.value.toLowerCase();Qe(e,G.filter(a=>{var i,r;return((i=a.nombre)==null?void 0:i.toLowerCase().includes(o))||((r=a.usuario)==null?void 0:r.toLowerCase().includes(o))}))}),e.querySelector("#btn-new").addEventListener("click",()=>Ge(e))}async function xe(e){try{G=await oe.listar(),e.querySelector("#total").textContent=`${G.length}`,e.querySelector("#footer-lbl").textContent=`${G.length} usuarios`,Qe(e,G)}catch(t){p(t.message,"error")}}function Qe(e,t){const o=e.querySelector("#tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="4">Sin usuarios registrados</td></tr>';return}o.innerHTML=t.map(a=>`
    <tr>
      <td><strong>${a.nombre}</strong></td>
    ...
    </tr>`).join(""),o.querySelectorAll(".edit-btn").forEach(a=>a.addEventListener("click",()=>Ge(e,G.find(i=>i.id==a.dataset.id)))),o.querySelectorAll(".del-btn").forEach(a=>a.addEventListener("click",()=>Tt(e,G.find(i=>i.id==a.dataset.id))))}function Ge(e,t=null){const o=wt.map(a=>`<option value="${a}" ${(t==null?void 0:t.rol)===a?"selected":""}>${a}</option>`).join("");N({title:t?"✏️ Editar Usuario":"+ Nuevo Usuario",bodyHTML:`
      <div class="form-grid">
        <div class="form-group">
          <label>Nombre completo *</label>
      ...
      </div>`,confirmText:t?"Actualizar":"Crear Usuario",onConfirm:async(a,i)=>{const r={nombre:a.querySelector("#m-nombre").value.trim(),usuario:a.querySelector("#m-usuario").value.trim(),contraseña:a.querySelector("#m-pass").value,rol:a.querySelector("#m-rol").value};if(!r.nombre||!r.usuario||!r.rol){p("Nombre, usuario y rol son obligatorios","warning");return}if(!t&&!r.contraseña){p("La contraseña es obligatoria","warning");return}try{t?await oe.actualizar(t.id,r):await oe.registrar(r),p(t?"Usuario actualizado":"Usuario creado","success"),i(),await xe(e)}catch(s){p(s.message,"error")}}})}function Tt(e,t){N({title:"🗑 Eliminar Usuario",bodyHTML:`<p style="color:var(--text2);line-height:1.7">¿Estás seguro de eliminar a <strong style="color:var(--text)">${t.nombre}</strong> (@${t.usuario})?<br/>Esta acción no se puede deshacer.</p>`,confirmText:"Eliminar",danger:!0,onConfirm:async(o,a)=>{try{await oe.eliminar(t.id),p("Usuario eliminado","success"),a(),await xe(e)}catch(i){p(i.message,"error")}}})}const xt="modulepreload",Ot=function(e){return"/"+e},Ce={},Me=function(t,o,a){let i=Promise.resolve();if(o&&o.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),n=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));i=Promise.allSettled(o.map(l=>{if(l=Ot(l),l in Ce)return;Ce[l]=!0;const d=l.endsWith(".css"),c=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${c}`))return;const u=document.createElement("link");if(u.rel=d?"stylesheet":xt,d||(u.as="script"),u.crossOrigin="",u.href=l,n&&u.setAttribute("nonce",n),document.head.appendChild(u),d)return new Promise((m,g)=>{u.addEventListener("load",m),u.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${l}`)))})}))}function r(s){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=s,window.dispatchEvent(n),!n.defaultPrevented)throw s}return i.then(s=>{for(const n of s||[])n.status==="rejected"&&r(n.reason);return t().catch(r)})},Oe={registrar:e=>x.post("/asistencia/alumnos",e),listar:()=>x.get("/asistencia/alumnos"),obtenerPorId:e=>x.get(`/asistencia/alumnos/${e}`),porAlumno:e=>x.get(`/asistencia/alumnos/alumno/${e}`),porFecha:e=>x.get(`/asistencia/alumnos/fecha?fecha=${e}`),porAlumnoYFecha:(e,t)=>x.get(`/asistencia/alumnos/alumno/${e}/fecha?fecha=${t}`),porGradoYFecha:(e,t)=>x.get(`/asistencia/alumnos/grado/${encodeURIComponent(e)}/fecha?fecha=${t}`)},ae=()=>new Date().toISOString().split("T")[0],Je=e=>e?new Date(e).toLocaleString("es-PE"):"—",Ye=e=>e==="ENTRADA"?'<span class="badge badge-green">▶ ENTRADA</span>':'<span class="badge badge-red">◀ SALIDA</span>';let U=[],z=[],We=[],X="personal";async function Dt(e){var o,a,i,r,s,n;const t=b.hasRole("ADMINISTRADOR","PORTERO","SECRETARIA");e.innerHTML=`
    <div class="page-topbar">
      <div>
        <h1>Asistencia</h1>
    ...
    </div>`,e.querySelectorAll(".tab-btn").forEach(l=>{l.addEventListener("click",()=>{X=l.dataset.tab,e.querySelectorAll(".tab-btn").forEach(d=>{d.classList.toggle("active",d.dataset.tab===X),d.classList.toggle("btn-primary",d.dataset.tab===X),d.classList.toggle("btn-outline",d.dataset.tab!==X)}),e.querySelector("#section-personal").style.display=X==="personal"?"":"none",e.querySelector("#section-alumnos").style.display=X==="alumnos"?"":"none"})});try{We=await oe.listar()}catch{}await Ie(e,ae()),(o=e.querySelector("#btn-filter"))==null||o.addEventListener("click",()=>{const l=e.querySelector("#filter-fecha").value;l&&Ie(e,l)}),(a=e.querySelector("#btn-all"))==null||a.addEventListener("click",()=>qt(e)),(i=e.querySelector("#btn-new"))==null||i.addEventListener("click",()=>Ct(e)),await Le(e,ae()),(r=e.querySelector("#btn-filter-alumnos"))==null||r.addEventListener("click",()=>{const l=e.querySelector("#filter-fecha-alumnos").value;l&&Le(e,l)}),(s=e.querySelector("#btn-all-alumnos"))==null||s.addEventListener("click",()=>Mt(e)),(n=e.querySelector("#btn-scan-qr"))==null||n.addEventListener("click",()=>Nt(e))}async function Ie(e,t){try{U=await Se.porFecha(t),e.querySelector("#total-personal").textContent=`${U.length}`,e.querySelector("#footer-personal").textContent=`${U.length} registros`,Ke(e,U)}catch{p("No se pudo conectar al servicio de asistencia (puerto 8081)","error")}}async function qt(e){try{U=await Se.listar(),e.querySelector("#total-personal").textContent=`${U.length}`,e.querySelector("#footer-personal").textContent=`${U.length} registros totales`,Ke(e,U)}catch(t){p(t.message,"error")}}function Ke(e,t){const o=e.querySelector("#tbody-personal");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="5">Sin registros para este filtro</td></tr>';return}o.innerHTML=t.map(a=>`
    <tr>
      <td><strong>${a.nombrePersonal}</strong></td>
      <td><span class="badge badge-purple">${a.rolPersonal}</span></td>
      <td>${Ye(a.tipoEvento)}</td>
      <td class="td-small">${Je(a.horaEvento)}</td>
      <td class="td-muted">${a.observaciones??"—"}</td>
    </tr>`).join("")}function Ct(e){const t=We.map(a=>`<option value="${a.id}" data-nombre="${a.nombre}" data-rol="${a.rol}">${a.nombre} — ${a.rol}</option>`).join(""),o=new Date().toISOString().slice(0,16);N({title:"+ Registrar Asistencia — Personal",bodyHTML:`
      <div class="form-grid">
        <div class="form-group" style="grid-column:1/-1">
      ...
      </div>`,confirmText:"Registrar",onConfirm:async(a,i)=>{const r=a.querySelector("#m-usuario"),s=r.options[r.selectedIndex],n=r.value;if(!n){p("Selecciona un miembro del personal","warning");return}const l={usuarioId:Number(n),nombrePersonal:s.dataset.nombre,rolPersonal:s.dataset.rol,tipoEvento:a.querySelector("#m-tipo").value,horaEvento:a.querySelector("#m-hora").value?new Date(a.querySelector("#m-hora").value).toISOString():null,observaciones:a.querySelector("#m-obs").value.trim()||null};try{await Se.registrar(l),p("Asistencia registrada","success"),i();const d=e.querySelector("#filter-fecha").value||ae();await Ie(e,d)}catch(d){p(d.message,"error")}}})}async function Le(e,t){try{z=await Oe.porFecha(t),e.querySelector("#total-alumnos").textContent=`${z.length}`,e.querySelector("#footer-alumnos").textContent=`${z.length} registros`,Xe(e,z)}catch{p("No se pudo cargar asistencia de alumnos","error")}}async function Mt(e){try{z=await Oe.listar(),e.querySelector("#total-alumnos").textContent=`${z.length}`,e.querySelector("#footer-alumnos").textContent=`${z.length} registros totales`,Xe(e,z)}catch(t){p(t.message,"error")}}function Xe(e,t){const o=e.querySelector("#tbody-alumnos");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="6">Sin registros para este filtro</td></tr>';return}o.innerHTML=t.map(a=>`
    <tr>
      ...
      <td class="td-small">${Je(a.horaEvento)}</td>
      <td class="td-muted">${a.observaciones??"—"}</td>
    </tr>`).join("")}function Nt(e){const t="qr-reader-"+Date.now();let o=null,a=!1;N({title:"📷 Escanear QR del Alumno",bodyHTML:`
      ...
      </div>`,confirmText:"Cerrar",hideCancelBtn:!0,onOpen:async s=>{try{const{Html5Qrcode:n}=await Me(async()=>{const{Html5Qrcode:d}=await import("./index-BNMS5zO2.js");return{Html5Qrcode:d}},[]);o=new n(t);const l={fps:10,qrbox:{width:220,height:220}};await o.start({facingMode:"environment"},l,d=>r(s,d),()=>{}),a=!0,s.querySelector("#scan-status").textContent="Esperando QR…"}catch(n){s.querySelector("#scan-status").textContent="No se pudo acceder a la cámara. "+(n.message||""),p("Error al iniciar la cámara","error")}},onClose:async()=>{if(o&&a)try{await o.stop()}catch{}}});let i=null;async function r(s,n){if(n!==i){if(i=n,o&&a)try{await o.pause()}catch{}s.querySelector("#scan-status").textContent="QR detectado — verificando alumno…";try{const{api:l}=await Me(async()=>{const{api:c}=await Promise.resolve().then(()=>ct);return{api:c}},void 0),d=await l.get(`/alumnos/qr/${n}`);s.querySelector("#scan-result").style.display="block",s.querySelector("#scan-nombre").textContent="🎒 "+d.nombre,s.querySelector("#scan-badges").innerHTML=`
        <span class="badge badge-blue">${d.grado}</span>
        <span class="badge badge-gray">${d.seccion}</span>`,s.querySelector("#scan-actions").style.display="block",s.querySelector("#scan-status").textContent="Selecciona el tipo de evento:",s.querySelector("#btn-entrada-qr").onclick=()=>Ne(s,n,"ENTRADA",e),s.querySelector("#btn-salida-qr").onclick=()=>Ne(s,n,"SALIDA",e)}catch{if(s.querySelector("#scan-status").textContent="❌ QR inválido — alumno no encontrado",p("QR no corresponde a ningún alumno","error"),i=null,o&&a)try{await o.resume()}catch{}}}}}async function Ne(e,t,o,a){var s,n,l;const i=((n=(s=e.querySelector("#scan-obs"))==null?void 0:s.value)==null?void 0:n.trim())||null,r={codigoQr:t,tipoEvento:o,horaEvento:null,registradoPorId:b.id()?Number(b.id()):null,observaciones:i};try{await Oe.registrar(r),p(`${o} del alumno registrada correctamente`,"success"),e.remove();const d=((l=a.querySelector("#filter-fecha-alumnos"))==null?void 0:l.value)||ae();await Le(a,d)}catch(d){p(d.message||"Error al registrar asistencia","error")}}const M={listarTodos:()=>S.get("/agenda"),listarPorProfesor:e=>S.get(`/agenda/profesor/${e}`),listarPorFecha:e=>S.get(`/agenda/fecha?fecha=${e}`),listarPorProfesorFecha:(e,t)=>S.get(`/agenda/profesor/${e}/fecha?fecha=${t}`),listarPorEstado:(e,t)=>S.get(`/agenda/profesor/${e}/estado/${t}`),listarRecurrentes:e=>S.get(`/agenda/recurrentes/${e}`),crear:e=>S.post("/agenda",e),actualizar:(e,t)=>S.put(`/agenda/${e}`,t),eliminar:e=>S.delete(`/agenda/${e}`),listarHorarios:()=>S.get("/horarios"),listarHorariosPorProfesor:e=>S.get(`/horarios/profesor/${e}`),listarHorariosPorDia:e=>S.get(`/horarios/dia/${e}`),crearHorario:e=>S.post("/horarios",e),actualizarHorario:(e,t)=>S.put(`/horarios/${e}`,t),eliminarHorario:e=>S.delete(`/horarios/${e}`)},Ze=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],Pt=["Dom","Lun","Mar","Mié","Jue","Vie","Sáb"],et=["DOMINGO","LUNES","MARTES","MIERCOLES","JUEVES","VIERNES","SABADO"],W={HORARIO_ATENCION:{bg:"#6366f1",light:"#eef2ff",text:"#4338ca",label:"Atención"},REUNION:{bg:"#f59e0b",light:"#fffbeb",text:"#b45309",label:"Reunión"},ACTIVIDAD:{bg:"#10b981",light:"#ecfdf5",text:"#047857",label:"Actividad"},OTRO:{bg:"#64748b",light:"#f1f5f9",text:"#334155",label:"Otro"}},me={ACTIVO:{bg:"#dcfce7",text:"#16a34a",label:"Activo"},CANCELADO:{bg:"#fee2e2",text:"#dc2626",label:"Cancelado"},COMPLETADO:{bg:"#e0e7ff",text:"#4338ca",label:"Completado"}},ye=e=>e?new Date(e).toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}):"—",Ht=e=>e?new Date(e).toLocaleDateString("es-PE",{day:"2-digit",month:"short",year:"numeric"}):"—",Ee=e=>e.toISOString().split("T")[0];let j=new Date().getFullYear(),q=new Date().getMonth(),B=[],K=[],$=null,Ae="calendario";async function kt(e){const t=b.hasRole("ADMINISTRADOR","DIRECTOR","PROFESOR"),o=b.hasRole("ADMINISTRADOR","DIRECTOR");e.innerHTML=Vt(t,o),Bt(e);const a=b.hasRole("PROFESOR")?b.id():null;await ie(a),_(e),Y(e),Ft(e)}function Vt(e,t){const o=b.hasRole("PROFESOR");return`
  <div class="page-topbar">
    <div>
  ...
  <div id="tab-horarios" class="tab-content" style="display:none">
    ...
    Eventos del ${i.getDate()} de ${Ze[i.getMonth()]} ${i.getFullYear()}`;const r=B.filter(s=>{const n=new Date(s.fechaInicio);return Ee(n)===t});if(!r.length){a.innerHTML='<div class="day-empty"><i class="fas fa-calendar-xmark"></i><span>Sin eventos este día</span></div>';return}a.innerHTML=r.map(s=>{const n=W[s.tipoAgenda]??W.OTRO,l=me[s.estado]??me.ACTIVO;return`
      ...
      </div>`:""}).join(""),e.querySelectorAll(".btn-edit-horario").forEach(l=>{l.addEventListener("click",()=>{const d=K.find(c=>c.id==l.dataset.id);d&&at(e,d)})}),e.querySelectorAll(".btn-del-horario").forEach(l=>{l.addEventListener("click",()=>ot(e,l.dataset.id,"horario"))})}function Bt(e){e.addEventListener("click",t=>{const o=t.target.closest(".agenda-tab");o&&(Ae=o.dataset.tab,e.querySelectorAll(".agenda-tab").forEach(a=>a.classList.toggle("active",a===o)),e.querySelectorAll(".tab-content").forEach(a=>{a.style.display=a.id===`tab-${Ae}`?"":"none"}),Ae==="horarios"&&Y(e))})}function Ft(e,t,o){var a,i,r,s,n,l,d,c;(a=e.querySelector("#cal-prev"))==null||a.addEventListener("click",()=>{q===0?(q=11,j--):q--,_(e),$&&J(e,$)}),(i=e.querySelector("#cal-next"))==null||i.addEventListener("click",()=>{q===11?(q=0,j++):q++,_(e),$&&J(e,$)}),(r=e.querySelector("#btn-filter-prof"))==null||r.addEventListener("click",async()=>{var m,g;const u=(g=(m=e.querySelector("#filter-profesor"))==null?void 0:m.value)==null?void 0:g.trim();await ie(u?Number(u):null),_(e),se(e),Y(e),$&&J(e,$)}),(s=e.querySelector("#btn-clear-filter"))==null||s.addEventListener("click",async()=>{const u=e.querySelector("#filter-profesor");u&&(u.value=""),await ie(null),_(e),se(e),Y(e),$&&J(e,$)}),(n=e.querySelector("#search-eventos"))==null||n.addEventListener("input",u=>{se(e,u.target.value)}),(l=e.querySelector("#filter-dia"))==null||l.addEventListener("change",u=>{Y(e,u.target.value)}),(d=e.querySelector("#btn-new-evento"))==null||d.addEventListener("click",()=>{tt(e,null)}),(c=e.querySelector("#btn-new-horario"))==null||c.addEventListener("click",()=>{at(e,null)}),se(e)}function tt(e,t){const o=!!t,a=o?"Editar Evento de Agenda":"Nuevo Evento de Agenda",i=b.hasRole("PROFESOR"),r=(t==null?void 0:t.profesorId)??(i?b.id():""),s=(t==null?void 0:t.nombreProfesor)??(i?b.nombre():""),n=d=>d?d.replace("T","T").substring(0,16):"",l=`
    <div class="form-grid-2">
      ...
      <div class="form-group" id="dia-semana-group" style="${t!=null&&t.recurrente?"":"display:none"}">
        ...
        <select id="ev-diaSemana" class="form-select">
          ${et.map(d=>`<option value="${d}" ${(t==null?void 0:t.diaSemana)===d?"selected":""}>${d.charAt(0)+d.slice(1).toLowerCase()}</option>`).join("")}
        </select>
      </div>
      <div class="form-group" style="grid-column:1/-1">
    ...
    <div class="form-grid-2">
      ...
      <div class="form-group">
        ...
        <select id="h-dia" class="form-select">
          ${et.map(c=>`<option value="${c}" ${(t==null?void 0:t.diaSemana)===c?"selected":""}>${c.charAt(0)+c.slice(1).toLowerCase()}</option>`).join("")}
        </select>
      </div>
      <div class="form-group">
        <label>Materia *</label>
    ...
    </div>`,await Gt(e);const t=e.querySelector("#aud-search"),o=e.querySelector("#aud-filter-rol"),a=()=>{const i=t.value.toLowerCase().trim(),r=o.value;let s=ce;r&&(s=s.filter(n=>n.rol===r)),i&&(s=s.filter(n=>{var l,d,c;return((l=n.usuario)==null?void 0:l.toLowerCase().includes(i))||((d=n.accion)==null?void 0:d.toLowerCase().includes(i))||((c=n.detalles)==null?void 0:c.toLowerCase().includes(i))})),it(e,s)};t.addEventListener("input",a),o.addEventListener("change",a)}async function Gt(e){try{ce=await jt.listar(),e.querySelector("#aud-total").textContent=`${ce.length}`,e.querySelector("#aud-footer-lbl").textContent=`${ce.length} transacciones registradas`,it(e,ce)}catch(t){p(t.message,"error")}}function it(e,t){const o=e.querySelector("#aud-tbody");if(!t.length){o.innerHTML='<tr class="empty-row"><td colspan="5">Sin transacciones registradas que coincidan</td></tr>';return}o.innerHTML=t.map(a=>`
    <tr>
      <td class="td-muted">${_t(a.fecha)}</td>
    ...
    </tr>`).join("")}const de=document.getElementById("app"),Jt={"#/login":{page:pt,auth:!1},"#/dashboard":{page:ft,auth:!0},"#/visitas":{page:Et,auth:!0},"#/visitantes":{page:St,auth:!0},"#/alumnos":{page:$t,auth:!0},"#/usuarios":{page:Lt,auth:!0,roles:["ADMINISTRADOR"]},"#/auditoria":{page:Qt,auth:!0,roles:["ADMINISTRADOR"]},"#/asistencia":{page:Dt,auth:!0},"#/agenda":{page:kt,auth:!0}};async function rt(){const e=window.location.hash||"#/dashboard",t=Jt[e];if(!t){window.location.hash="#/dashboard";return}if(!t.auth){if(b.isLogged()){window.location.hash="#/dashboard";return}de.innerHTML="",t.page(de);return}if(!b.isLogged()){window.location.hash="#/login";return}if(t.roles&&!t.roles.includes(b.rol())){window.location.hash="#/dashboard";return}de.innerHTML=`
    <div class="app-container">
      <div id="sidebar-wrap"></div>
      <div class="main-content" id="page-root"></div>
    </div>`,dt(de.querySelector("#sidebar-wrap"));const o=de.querySelector("#page-root");try{await t.page(o)}catch(a){o.innerHTML=`
      ...
      </div>`}}window.addEventListener("hashchange",rt);window.addEventListener("load",()=>{window.location.hash||(window.location.hash="#/dashboard"),rt()});
